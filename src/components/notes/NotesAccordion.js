...
function NotesAccordion({ note, setNote, editing, allNotes, setAllNotes, setEditing, handleSaveNote, handleUpdateNote }) {
  ...
  const handleDeleteNote = (id) => {
    setAllNotes(allNotes.filter(noteItem => noteItem.id !== id)); // Filter based on noteItem.id
  };
      ...
      <GridItem colSpan={1} rowSpan={1}>
        <div className="container">
          ...
          {allNotes
            ...
            .map(noteItem => {
              if (noteItem.id === 0) return <React.Fragment key={noteItem.id} />;
