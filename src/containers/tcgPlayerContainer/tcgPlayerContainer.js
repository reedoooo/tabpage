...
function TcgPlaerContainer({ task }) {
  ...
  useEffect(() => {
    const getToken = async () => {
      try {
        const response = await axios.post(`${process.env.REACT_APP_SERVER}/api/token`);
  ...
  const getCategories = async () => {
    ...
    try {
      const response = await axios.get(`${process.env.REACT_APP_SERVER}/api/catalog/categories`, {
        headers: {
